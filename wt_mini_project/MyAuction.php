  ...
  <div class="row">
      ...
      <div class="container">
        <?php
                ...
                $sql = "SELECT DISTINCT b.`Sname`,b.`Pid`,b.`Bid`,b.`Bname`,p.`name`,p.`time`,p.`sold_status` FROM `bid` as b ,`product` as p Where Bid in(SELECT DISTINCT max(Bid) FROM `bid` WHERE Bname='$name' GROUP by Pid) and b.pid=p.pid and Bname='$name'";
                ...
                <thead>
                    ...
                    while ($row = mysqli_fetch_row($result)) { 
                      $time=str_replace("t"," ",$row[5]); 
                        ...
                        <th>".$row[2]."</th>
                        <th>$status</th>
                        <th><a  style='color: Blue;' href='http://localhost/wt_mini_project/buyerppage.php?data=".$name."&pid=".$row[1]."' class='product-link'>Link</a></th>
                    </tr>";
